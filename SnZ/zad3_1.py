...
if __name__ == "__main__":
    ...
    trainingData1 = trainingData[:int(len(trainingData) / 2)]
    trainingData2 = trainingData[int(len(trainingData) / 2):]

    tree1 = build_tree(trainingData1, entropy)
    tree2 = build_tree(trainingData2, entropy)

    result1 = classify(testCase, tree1)
    result2 = classify(testCase, tree2)

    class1 = sorted(list(result1.items()),
                    key=lambda x: x[1],
                    reverse=True)[0][0]

    class2 = sorted(list(result2.items()),
                    key=lambda x: x[1],
                    reverse=True)[0][0]
