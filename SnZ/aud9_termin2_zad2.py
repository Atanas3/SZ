...
if __name__ == '__main__':
    ...
    from tf_idf import get_words, calculate_document_frequencies,\
        rank_documents, cosine, pearson

    documents = [x[0] for x in train_data]
    ...
    vocab = [word[0] for word in sorted_frequencies[100:]]
    print(sorted_frequencies[100:])

    df = calculate_document_frequencies(documents, vocab)

    res_cosine = rank_documents(text, documents, vocab=vocab, df=df, sim_func=cosine)[:5]
    res_pearson = rank_documents(text, documents, vocab=vocab, df=df, sim_func=pearson)[:5]

    print(res_cosine)
    print(res_pearson)

    for i, r_cosine, r_pearson in zip(range(5), res_cosine, res_pearson):
        print(f'Rang po slicnost:{i} Cosine-Document #{r_cosine[1]} Pearson-Document #{r_pearson[1]}')
