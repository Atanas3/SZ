...
words_to_ignore=['and', 'are', 'for', 'was', 'what',
                 'when', 'who', 'but', 'from',
                 'after', 'out', 'our', 'my', 'the',
...
if __name__ == "__main__":
    ...
    from document_classification_modified import get_words, NaiveBayes

    classifier1 = NaiveBayes(get_words)
    for train_data_el in train_data:
        classifier1.train(train_data_el[0], train_data_el[1])
    class1 = classifier1.classify_document(recenica)

    classifier2 = NaiveBayes(get_words)
    for train_data_el in train_data:
        classifier2.train(train_data_el[0], train_data_el[1], words_to_ignore)
    class2 = classifier2.classify_document(recenica, words_to_ignore)
