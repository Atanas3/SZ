...
from tf_idf import calculate_document_frequencies, get_vocabulary, process_document
...
if __name__ == '__main__':
    ...
    for i in sentences:
        sent = documents[i]
        tf_idf = process_document(sent, df, N, vocab)
        pom = []
        for j in range(len(vocab)):
            val = tf_idf[j]
            if val > threshold:
                pom.append((vocab[j], val))
        if len(pom) == 0:
            print(sent, 'No keywords...')
        keywords = list(sorted(pom, key=lambda x: x[1], reverse=True))[:5]
        print_str = ''
        for keyword in keywords:
            print_str += keyword[0] + ': ' + str(keyword[1]) + ', '
        print(sent, '->', print_str[:-2])