...
if __name__ == '__main__':
    ...
    classifer1 = NaiveBayes(get_words)
    classifer2 = NaiveBayes(get_words)

    for train_sample in train_set_1:
        classifer1.train(train_sample[0], train_sample[1])

    for train_sample in train_set_2:
        classifer2.train(train_sample[0], train_sample[1])

    corrrect_samples = 0

    for test_sample in test_set:
        true_class = test_sample[1]
        class1 = classifer1.classify_document(test_sample[0])
        class2 = classifer2.classify_document(test_sample[0])

        print(test_sample[0])

        if true_class == class1:
            print('Klasifikatorot 1 uspeshno ja odredi klasata')
        else:
            print('Klasifikatorot 1 ne ja odredi klasata')

        if true_class == class2:
            print('Klasifikatorot 2 uspeshno ja odredi klasata')
        else:
            print('Klasifikatorot 2 ne ja odredi klasata')

        if true_class == class1 and true_class == class2:
            corrrect_samples += 1

    accuracy = corrrect_samples / len(test_set)
    print(accuracy)

    class1 = classifer1.classify_document(sample)
    class2 = classifer2.classify_document(sample)
