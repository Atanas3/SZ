...
if __name__ == "__main__":
    ...
    data_roach = [d for d in data if d[-1] == 'Roach'][:40]
    data_pike = [d for d in data if d[-1] == 'Pike'][:40]

    trainingData = data_roach + data_pike

    tree = build_tree(trainingData, entropy)

    result = classify(test_case, tree)
    class1 = sorted(list(result.items()),
                    key=lambda x: x[1],
                    reverse=True)[0][0]
