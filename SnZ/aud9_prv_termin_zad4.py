...
def get_words(doc):
    ...
    words = list()
    for word in re.split('\\W+', doc):
        if 2 < len(word) < 20 and word.lower() not in top100:
            words.append(word.lower())
    return words
...
def calculate_document_frequencies2(documents):
    """Враќа речник со број на појавување на зборовите.
...
if __name__ == '__main__':
    ...
    df = calculate_document_frequencies2(documents)

    zborovi = df.items()
    sort_zborovi = sorted(zborovi, key=lambda x: x[0])
    sort_zborovi2 = list(sorted(sort_zborovi, key=lambda x: x[1], reverse=True))
    top100 = [x[0] for x in sort_zborovi2][:100]

    perason = rank_documents(doc, documents, pearson)[:5]
    cosiine_vals = rank_documents(doc, documents, cosine)[:5]

    for i in range(5):
        print(f'Rang po slicnost:{i} Cosine-Document #{cosiine_vals[i][1]} Pearson-Document #{perason[i][1]}')
